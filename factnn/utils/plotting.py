...
def plot_roc(truth, predictions, ax=None):
    ...
    ax.set_title("Area Under Curve: {:.4f}".format(roc_auc_score(truth, predictions)))
...
def plot_probabilities(
    performace_df, model=None, ax=None, classnames=("Proton", "Gamma")
):
    ax = ax or plt.gca()

    bin_edges = np.linspace(0, 1, 100 + 2)
    ax.hist(
        performace_df, bins=bin_edges, label="Proton", histtype="step",
    )
    if model is not None:
        ax.hist(
            model, bins=bin_edges, label="Gamma", histtype="step",
        )
