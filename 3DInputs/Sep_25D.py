            ...
            while True:
                ...
                while batch_size * (batch_num + 1) < items:
                    ...
                    if type_training == 'Energy':
                        ...
                        batch_images = batch_images.reshape((-1,75,75,25))
                        proton_images = np.swapaxes(proton_images, 1, 3)
                        proton_images = proton_images.reshape((-1,75,75,25))
...
def create_model(batch_size, patch_size, dropout_layer, num_dense, num_conv, num_pooling_layer, dense_neuron, conv_neurons, frac_per_epoch,
    ...
    model_name = "MC_Seperation2DSpatial" + "_p_" + str(
    ...
    if not os.path.isfile(model_base + model_name + ".csv"):
        ...
        reduceLR = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=9, min_lr=0.0001)
        ...
        early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0,
                                                   patience=10,
                                                   verbose=0, mode='auto')
        tb = keras.callbacks.TensorBoard(log_dir='/run/media/jacob/WDRed8Tb1/TensorBoardLogs2/', histogram_freq=1, batch_size=8, write_graph=True,
                                         write_grads=True,
                                         write_images=True,
                                         embeddings_freq=0,
        ...
        model.add(Conv2D(64, kernel_size=5, strides=2,
                             padding='same',
                             input_shape=(75, 75, 25), activation='elu'))
        model.add(Dropout(0.5))
        model.add(
            Conv2D(128, kernel_size=(5,5), strides=2,
                   padding='same', activation='elu'))
        model.add(Dropout(0.5))
        model.add(
            Conv2D(256, kernel_size=(5,5), strides=2,
                   padding='same', activation='elu'))
        #model.add(Dropout(0.3))
        #model.add(Conv2D(8, kernel_size=3, strides=2,
        #                     padding='same', activation='relu', recurrent_activation='hard_sigmoid'))
        model.add(Dropout(0.5))
        model.add(Flatten())
        ...
        model.fit_generator(generator=batchYielder(path_to_training_data=path_mc_images, time_slice=time_slice, path_to_proton_data=path_proton_images, type_training="Separation", batch_size=batch_size, percent_training=0.6),
                            ...
                            , epochs=1600,
                            verbose=1, validation_data=validationGenerator(0.2, time_slice=time_slice, batch_size=128*batch_size),
