...
class CLS_SSG_Model(Model):
    def __init__(
        self, batch_size, num_points, num_classes, bn=False, activation=tf.nn.leaky_relu
    ):
    ...
    def init_network(self):
        ...
        if self.bn:
            self.bn_fc2 = BatchNormalization()
    ...
    def call(self, input, training=True):
        ...
        net = self.dense1(net)
        if self.bn:
            net = self.bn_fc1(net, training=training)
        net = self.dropout1(net)

        net = self.dense2(net)
        if self.bn:
            net = self.bn_fc2(net, training=training)
...
model = CLS_SSG_Model(16, final_points, 2, False)
...
callbacks = list()
callbacks.append(
    tf.keras.callbacks.TensorBoard(log_dir="./", histogram_freq=0, write_graph=True)
)
callbacks.append(
