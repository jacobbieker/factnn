...
def write_dataset(base="/run/media/bieker/T7/", split="train"):
    ...
    core = "/run/media/bieker/T7/gamma/core5/raw/"
    clump = "/run/media/bieker/T7/gamma/clump5/raw/"
    puncleaned = "/run/media/bieker/T7/proton/no_clean/raw/"
    pcore = "/run/media/bieker/T7/proton/core5/raw/"
    pclump = "/run/media/bieker/T7/proton/clump5/raw/"
    ...
    event_dict = pickle.load(  # Only need one
        open(
            res.resource_filename(
                "factnn.data.resources", f"core5_raw_names.p"
            ),
            "rb",
        )
    )
    ...
    pattern = os.path.join(base, f"fact-{split}-%07d.tar")
