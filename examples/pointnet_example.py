...
def transform_net(inputs, scope=None, regularize=False):
    ...
    with K.name_scope(scope):
        ...
        net = MaxPooling2D(pool_size=(num_points, 1), padding="valid")(
        ...
        transform = Dense(
            ...
            bias_initializer=Constant(np.eye(k).flatten()),
            activity_regularizer=orthogonal(l2=0.001) if regularize else None,
        )(net)
...
def pointnet_base(inputs, use_tnet=True):
    ...
    net = conv1d_bn(
        point_cloud_transformed if use_tnet else inputs,
        num_filters=128,
        kernel_size=1,
        padding="valid",
        use_bias=True,
    ...
    net = conv1d_bn(
        net_transformed if use_tnet else net,
        num_filters=64,
        kernel_size=1,
        padding="valid",
        use_bias=True,
...
def pointnet_cls(
    ...
    if include_top:
        ...
        # Done in 2D since 1D is painfully slow
        net = MaxPooling2D(pool_size=(num_point, 1), padding="valid", name="maxpool")(
            Lambda(K.expand_dims)(net)
        )
        net = Flatten()(net)
        ...
        if pooling == "avg":
            net = MaxPooling2D(
                pool_size=(num_point, 1), padding="valid", name="maxpool"
            )(Lambda(K.expand_dims)(net))
        elif pooling == "max":
            net = AveragePooling2D(
                pool_size=(num_point, 1), padding="valid", name="avgpool"
            )(Lambda(K.expand_dims)(net))
...
paths = paths  # [:5000]
train_paths = paths[: int(0.6 * len(paths))]
...
train_p_paths = paths[: int(0.6 * len(proton_paths))]
val_p_paths = paths[int(0.6 * len(proton_paths)) : int(0.8 * len(proton_paths))]
test_p_paths = paths[int(0.8 * len(proton_paths)) :]

gamma_configuration = {"paths": train_paths, "rebin_size": 5, "shape": (0, 160)}
proton_configuration = {"paths": train_p_paths, "rebin_size": 5, "shape": (0, 160)}
...
val = PointCloudGenerator(
    val_paths,
    batch_size=16,
    preprocessor=gamma_val_preprocessor,
...
model = pointnet_cls(
    input_shape=(final_points, 3), classes=2, use_tnet=True, activation="softmax"
)
...
callbacks = list()
callbacks.append(
    keras.callbacks.TensorBoard(log_dir="./", histogram_freq=0, write_graph=True)
)
callbacks.append(
    keras.callbacks.ReduceLROnPlateau(
        monitor=monitor, factor=0.5, patience=5, verbose=1, min_lr=1e-10
    )
)
