...
def write_dataset(base="/run/media/jacob/data/FACT_Dataset/", split="proton", num=20):
    pattern = os.path.join(base, f"fact-{split}-{num}-%04d.tar")
    num_examples_per_shard = 10000
    dataset = wds.WebDataset("/run/media/jacob/data/FACT_Dataset/fact-train-20-{0000..0058}.tar").decode()
    test_dataset = wds.WebDataset("/run/media/jacob/data/FACT_Dataset/fact-test-20-{0000..0019}.tar").decode()
    val_dataset = wds.WebDataset("/run/media/jacob/data/FACT_Dataset/fact-val-20-{0000..0019}.tar").decode()
    with ShardWriter(pattern, maxcount=num_examples_per_shard, compress=True) as sink:
...
write_dataset(split='proton')
write_dataset(split="gamma")
