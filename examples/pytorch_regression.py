...
def calc_confsion_matrix(prediction, truth, log_xy=True):
    ...
    limits = [min_ax, max_ax]

    counts, x_edges, y_edges = np.histogram2d(
        truth, prediction, bins=[100, 100], range=[limits, limits]
    )
    return counts
...
"""
    ...
    if args.task == "energy":
        logger.report_confusion_matrix(
            ...
            iteration=epoch,
            xaxis=r"$\log_{10}(E_{\mathrm{MC}} \,\, / \,\, \mathrm{GeV})$",
...
def default_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="no_clean",
        help="cleanliness value, one of 'no_clean', 'clump5','clump10', 'clump15', 'clump20', "
    ...
    parser.add_argument(
        ...
        type=str,
        default="mse",
        help="loss type, currently mse or mae, default mse",
    )
    parser.add_argument(
        "--task",
        type=str,
...
if __name__ == "__main__":
    ...
    if (
        args.augment and args.task == "energy"
    ):  # Currently haven't figured out rotating the Phi, Theta, and Disp values if points are moved
