...
from keras.layers import Dense, Dropout, BatchNormalization, Activation, Flatten, ConvLSTM2D, Conv2D, Conv1D, MaxPooling1D, Conv3D, MaxPooling3D, MaxPooling2D, LSTM, Reshape
...
protons = read_h5py("proton.hdf5", key="events", columns=columns, last=240000)
gammas = read_h5py("gamma.hdf5", key="events", columns=columns, last=240000)
...
'''
model.add(
    Conv1D(32, input_shape=(250,), kernel_size=3, strides=1,
...
model.add(Dropout(0.3))
model.add(Dense(512, input_shape=(250,), activation='relu'))
model.add(BatchNormalization())
model.add(Dense(256, activation='relu'))
model.add(Dropout(0.3))
model.add(BatchNormalization())
...
model_checkpoint = keras.callbacks.ModelCheckpoint("featureSepTest.h5",
                                                   monitor='val_loss',
                                                   verbose=0,
                                                   save_best_only=True,
