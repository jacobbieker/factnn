...
path_mc_images = "/run/media/jacob/WDRed8Tb2/Rebinned_5_MC_gamma_SOURCEXYREALALLSTDDEV_Images.h5"
...
with h5py.File(path_mc_images, 'r') as f:
    ...
    #disp_test = true_disp[-int(0.2*len(images)):]
    sign_test = true_sign[-int(0.2*len(images)):]
    ...
    sign_train = true_sign[0:int(0.8*len(true_sign))]
    ...
    #print(source_x[0])
    #print(source_y[0])
    y_label = sign_train#[1000:-1]
    x_label = sign_test#sign_train#[1000:-1]
    print(y_label[0])
    print("Y values: ")
...
def create_model(batch_size, patch_size, dropout_layer, num_dense, num_conv, num_pooling_layer, dense_neuron, conv_neurons, optimizer):
    try:
        ...
        model_name = "MC_OneOutputPoolSIGNCONV" + "_drop_" + str(dropout_layer) + "_convNeruon_" + str(conv_neurons)
        if not os.path.isfile(model_base + model_name + ".csv"):
            ...
            y_out = Dense(2, name="y_out", activation='softmax')(y)
