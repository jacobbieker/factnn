...
with h5py.File(path_mc_images, 'r') as f:
    ...
    y = images
    y_label = np.asarray(images_source_zd).reshape(-1, 1)
    print(y_label.shape)
                                            ...
                                            with h5py.File(path_mc_images, 'r') as f:
                                                ...
                                                while True:
                                                    ...
                                                    while (batch_size) * (batch_num + 1) < items:
                                                        ...
                                                        images_source_zd = f['Energy'][batch_num*batch_size:(batch_num+1)*batch_size]
                                                        x = images
                                                        x_label = np.asarray(images_source_zd).reshape((-1, 1))
                                                        #print(x_label)
                                                        batch_num += 1
                                                        yield (x, x_label)
