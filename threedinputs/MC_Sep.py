...
def validationGenerator(validation_percentage, time_slice=100, batch_size=64, predicting_labels=[], predicting=False):
...
all_labels = []
def predictGenerator(validation_percentage, time_slice=100, batch_size=64):
    with h5py.File(path_mc_images, 'r') as f:
        with h5py.File(path_proton_images, 'r') as f2:
            # Get some truth data for now, just use Crab images
            items = len(f2["Image"])
...
model = keras.models.load_model("/run/media/jacob/WDRed8Tb1/Models/3DSep/_MC_Seperation3DSpatial_p_(5, 5)_drop_0.64_numDense_3_conv_3_pool_1_denseN_219_convN_94.h5")
...
predictions = model.predict_generator(predictGenerator(0.4, time_slice=time_slice, batch_size=4), steps=int(np.floor(0.4*length_items/4)))
...
def create_model(batch_size, patch_size, dropout_layer, num_dense, num_conv, num_pooling_layer, dense_neuron, conv_neurons, frac_per_epoch):
    #try:
        ...
        if not os.path.isfile(model_base + model_name + ".csv"):
            ...
            reduceLR = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=110, min_lr=0.0001)
            ...
            early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0,
                                                       patience=120,
                                                       verbose=0, mode='auto')
            #tb = keras.callbacks.TensorBoard(log_dir='/run/media/jacob/WDRed8Tb1/TensorBoardLogs/', histogram_freq=0, batch_size=16, write_graph=True,
            #                            write_grads=True,
            #                            write_images=True,
            #                            embeddings_freq=0,
            ...
            # Make the model
            model = Sequential()
            regularize = keras.regularizers.l1(0.004)
            regularizel2 = keras.regularizers.l2(0.0004)
            # Base Conv layer
            model.add(ConvLSTM2D(8, kernel_size=5, strides=4,
                                 padding='same',
                                 input_shape=(total_slices, 75, 75, 1), activation='elu', recurrent_regularizer=regularize,  dropout=0.4, recurrent_dropout=0.3, recurrent_activation='hard_sigmoid'))
            #model.add(
            #    ConvLSTM2D(64, kernel_size=3, strides=2,  dropout=0.2, recurrent_dropout=0.1,
            #               padding='same', activation='tanh', recurrent_activation='hard_sigmoid', return_sequences=True))
            #model.add(BatchNormalization())
            #model.add(
            #    ConvLSTM2D(64, kernel_size=3, strides=2, recurrent_regularizer=regularize, dropout=0.4, recurrent_dropout=0.3,
            #               padding='same', recurrent_activation='hard_sigmoid', activation='elu'))
            ...
            model.fit_generator(generator=batchYielder(path_to_training_data=path_mc_images, time_slice=time_slice, path_to_proton_data=path_proton_images, type_training="Separation", batch_size=batch_size, percent_training=0.6),
                                ...
                                , epochs=1600,
                                verbose=2, validation_data=validationGenerator(0.2, time_slice=time_slice, batch_size=batch_size), validation_steps=int(np.floor(0.02*length_items/16)),
                                callbacks=[early_stop, reduceLR, model_checkpoint],
                                )
            predictions = model.predict_generator(validationGenerator(0.4, time_slice=time_slice, predicting=True, batch_size=batch_size), steps=int(np.floor(0.4*length_items/16)))
